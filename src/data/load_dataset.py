...
class LoadData():
    def __init__(self) -> None:
        # file_path = os.path.relpath(os.path.realpath('logs/data_logs.log'), os.path.relpath(os.path.realpath(__file__)))
    ...
    def _get_spectogram(self, waveform):
        spectrogram = tf.signal.stft(waveform, frame_length=255, frame_step=128)
        spectrogram = tf.abs(spectrogram)
        spectrogram = spectrogram[..., tf.newaxis]
        return spectrogram
    
    def convert_to_spectogram(self, data_set):
        """
        Converts audio data to spectograms.
        ...
        `_get_spectogram` method to perform the conversion.

        Example:
        ```
        audio_dataset = ...
        spectrogram_dataset = self.convert_to_spectogram(audio_dataset)
        ```

        """
        return data_set.map(
            map_func=lambda audio,label: (self._get_spectogram(audio), label),
